...
this.ifStack =  Clazz.newArray (8, '\0');
});
...
this.skipping = (this.ifPt >= 0 && ((this.ifStack[this.ifPt]).charCodeAt (0) == ('F').charCodeAt (0) || (this.ifStack[this.ifPt]).charCodeAt (0) == ('X').charCodeAt (0)));
if (this.skipping) {
switch (op.tok) {
case 269484048:
this.putOp (op);
return true;
case 269484066:
if (tok0 != 269484066 || (this.ifStack[this.ifPt]).charCodeAt (0) == ('X').charCodeAt (0)) return true;
...
break;
case 806354977:
var isFirst = org.jmol.script.ScriptVariable.bValue (this.getX ());
...
this.skipping = (this.ifPt >= 0 && (this.ifStack[this.ifPt]).charCodeAt (0) == ('X').charCodeAt (0));
return true;
case 269484080:
this.wasX = false;
return true;
...
var list = x1.getList ();
data =  Clazz.newArray (list.size (), 0);
for (var i = list.size (); --i >= 0; ) data[i] = org.jmol.script.ScriptVariable.fValue (list.get (i));

}var nbins = Math.round (((f1 - f0) / df + 0.01));
var array =  Clazz.newArray (nbins, 0);
var nPoints = data.length;
for (var i = 0; i < nPoints; i++) {
var v = data[i];
var bin = Math.round (((v - f0) / df));
if (bin < 0) bin = 0;
...
return (Clazz.instanceOf (pt, javax.vecmath.Point4f) ? pt : null);
case 10:
break;
}
return null;
...
if (s.equalsIgnoreCase ("vdw") || s.equalsIgnoreCase ("vanderwaals")) vdw = (i + 1 < args.length && args[i + 1].tok == 2 ? org.jmol.script.ScriptVariable.iValue (args[++i]) : 100) / 100;
...
sList1 = (org.jmol.util.TextFormat.split (x1.value, "\n"));
list1 =  Clazz.newArray (len = sList1.length, 0);
org.jmol.util.Parser.parseFloatArray (sList1, list1);
}if (isAll) {
var sum = 0;
if (x1.tok == 7) {
...
sList2 = org.jmol.util.TextFormat.split (x2.value, "\n");
list2 =  Clazz.newArray (sList2.length, 0);
org.jmol.util.Parser.parseFloatArray (sList2, list2);
...
($fz = function (args, tok) {
if (args.length != 1) return false;
var n = org.jmol.script.ScriptVariable.iValue (args[0]) - 1;
var x1 = this.getX ();
var f;
switch (x1.tok) {
case 11:
...
if (isMatrix) {
var m =  Clazz.newArray (len * len, 0);
var pt = 0;
...
nMax = org.jmol.script.ScriptVariable.iValue (args[nArgs - 1]);
...
var data1 = org.jmol.script.ScriptMathProcessor.getQuaternionArray (args[0].getList ());
...
var data1 = org.jmol.script.ScriptMathProcessor.getQuaternionArray (args[0].getList ());
var data2 = org.jmol.script.ScriptMathProcessor.getQuaternionArray (args[1].getList ());
...
if (args.length == 4) {
var iField = org.jmol.script.ScriptVariable.iValue (args[1]);
...
case 1048589:
case 1048588:
isWithinModelSet = org.jmol.script.ScriptVariable.bValue (args[1]);
i = 0;
break;
case 4:
var s = org.jmol.script.ScriptVariable.sValue (args[1]);
...
i = args.length - 1;
if (Clazz.instanceOf (args[i].value, javax.vecmath.Point4f)) {
plane = args[i].value;
...
haveDecimal = true;
default:
var n = org.jmol.script.ScriptVariable.iValue ($var);
var f = org.jmol.script.ScriptVariable.fValue ($var);
...
case 269484210:
s = null;
var n = org.jmol.script.ScriptVariable.iValue (x2);
switch (x1.tok) {
case 1048589:
case 1048588:
case 2:
...
var ndata = 0;
while (true) {
if (Clazz.instanceOf (floatOrSVArray, Array)) {
data = floatOrSVArray;
ndata = data.length;
if (ndata == 0) break;
...
if (Clazz.instanceOf (pt, javax.vecmath.Point3f)) return org.jmol.script.ScriptMathProcessor.getMinMaxPoint (sv, tok);
if (Clazz.instanceOf (pt, javax.vecmath.Point4f)) return org.jmol.script.ScriptMathProcessor.getMinMaxQuaternion (sv, tok);
break;
}} else {
break;
}var sum;
...
var result =  new javax.vecmath.Point3f ();
var fdata =  Clazz.newArray (ndata, 0);
var ok = true;
...
($fz = function (quaternionOrSVData, tok) {
var data;
switch (tok) {
case 32:
case 64:
...
while (true) {
data = org.jmol.script.ScriptMathProcessor.getQuaternionArray (quaternionOrSVData);
if (data == null) break;
var retStddev =  Clazz.newArray (1, 0);
...
function (quaternionOrSVData) {
var data;
if (Clazz.instanceOf (quaternionOrSVData, Array)) {
