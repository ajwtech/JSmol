...
var className = org.jmol.viewer.JmolConstants.getShapeClassName (shapeID);
try {
var shapeClass = Class.forName (className);
var shape = shapeClass.newInstance ();
this.viewer.setShapeErrorState (shapeID, "allocate");
shape.initializeShape (this.viewer, this.g3d, this.modelSet, shapeID);
this.viewer.setShapeErrorState (-1, null);
...
var cmd;
for (var i = 0; i < 35; ++i) {
if (iShape != 2147483647 && i != iShape) continue ;var shape = this.shapes[i];
...
var minZ = this.g3d.getSlab ();
var maxZ = this.g3d.getDepth ();
if (slabByMolecule) {
...
if (!this.bsRenderableAtoms.get (pt)) continue ;for (; j < m.atomCount; j++, pt++) if (this.g3d.isClippedZ (atoms[pt].screenZ - (atoms[pt].screenDiameter >> 1))) break;
...
var atom = atoms[i];
if (this.g3d.isClippedZ (atom.screenZ - (slabByAtom ? atoms[i].screenDiameter >> 1 : 0))) {
atom.setClickable (0);
var r = Math.floor ((slabByAtom ? -1 : 1) * atom.screenDiameter / 2);
if (atom.screenZ + r < minZ || atom.screenZ - r > maxZ || !this.g3d.isInDisplayRange (atom.screenX, atom.screenY)) {
