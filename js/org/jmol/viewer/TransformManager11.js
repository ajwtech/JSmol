...
this.haveNotifiedNaN = true;
}this.point3iScreenTemp.set (Math.round (this.point3fScreenTemp.x), Math.round (this.point3fScreenTemp.y), Math.round (this.point3fScreenTemp.z));
});
...
var pts =  new javax.vecmath.Point3f ();
...
if (key != null) this.viewer.getGlobalSettings ().setParameterValue (key, value);
...
var fps = 30;
var totalSteps = Math.round ((floatSecondsTotal * fps));
if (floatSecondsTotal > 0) this.viewer.setInMotion (true);
if (degrees == 0) degrees = NaN;
if (totalSteps > 1) {
var frameTimeMillis = Math.floor (1000 / fps);
...
var sleepTime = Math.round ((floatSecondsTotal * 1000)) - 30;
if (sleepTime > 0) {
try {
Thread.sleep (sleepTime);
} catch (ie) {
...
var nPer = Math.round ((10 * seconds));
