...
org.jmol.util.GData.setZShadePower (this.global.zShadePower);
...
this.gdata.setZShade (this.transformManager.zShadeEnabled, this.transformManager.zSlabValue, this.transformManager.zDepthValue);
});
...
if (type == null) {
type = org.jmol.util.SurfaceFileTyper.determineSurfaceFileType (this.getBufferedInputStream (fileName));
if (type != null) {
this.evalString ("if (_filetype == 'Pdb') { isosurface sigma 1.0 within 2.0 {*} " + org.jmol.util.Escape.escape (fileName) + " mesh nofill }; else; { isosurface " + org.jmol.util.Escape.escape (fileName) + "}");
return ;
...
return ;
 ...
 else this.evalString ((cmd == null ? "script " : cmd) + org.jmol.util.Escape.escape (fileName));
}, "~S,~B");
...
return this.fileManager.createZipSet (null, s, scripts, true);
}try {
...
this.finalizeTransformParameters ();
var minMax = this.shapeManager.transformAtoms (this.transformManager.bsSelectedAtoms, this.transformManager.ptOffset);
...
var image = (this.transformManager.stereoMode.isBiColor () ? this.getStereoImage (this.transformManager.stereoMode) : this.getImage (this.isStereoDouble ()));
var image1 = null;
if (mergeImages) {
image1 = this.apiPlatform.newBufferedImage (image, this.dimScreen.width << 1, this.dimScreen.height);
graphic = this.apiPlatform.getGraphics (image1);
}if (graphic != null) {
if (this.isStereoDouble ()) {
this.render1 (graphic, image, this.dimScreen.width, 0);
image = this.getImage (false);
}this.render1 (graphic, image, 0, 0);
...
if (type.equals ("jpg64") || type.equals ("jpeg64")) bytes = (bytes == null ? "" : org.jmol.util.Base64.getBase64 (bytes).toString ());
} catch (er) {
if (Clazz.instanceOf (er, Error)) {
...
function (object) {
return org.jmol.util.Colix.getColix (object);
}, "~O");
...
break;
case 553648190:
org.jmol.util.GData.setZShadePower (Math.max (value, 1));
break;
case 553648166:
...
return this.gdata.getFont3D (fontFace, fontStyle, fontSize);
}, "~S,~S,~N");
...
err = this.fileManager.createZipSet (fileName, text_or_bytes, scripts, type.equals ("ZIPALL"));
...
return g3dExport.initializeExporter (type, this, this.privateKey, this.gdata, output);
...
if (iScene <= iSceneLast) continue ;iSceneLast = iScene;
str[2] = "all";
var fileName = fileRoot + "_scene_" + iScene + ".all." + fileExt;
var msg = this.createImage (fileName, "PNGJ", null, str, -1, width, height, null, false);
str[0] = null;
str[2] = "min";
fileName = fileRoot + "_scene_" + iScene + ".min." + fileExt;
msg += "\n" + this.createImage (fileName, "PNGJ", null, str, -1, Math.min (width, 200), Math.min (height, 200), null, false);
