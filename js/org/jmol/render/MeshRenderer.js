...
this.latticeOffset =  new javax.vecmath.Point3f ();
this.pt1f =  new javax.vecmath.Point3f ();
this.pt2f =  new javax.vecmath.Point3f ();
...
var mc = this.shape;
for (var i = mc.meshCount; --i >= 0; ) this.render1 (mc.meshes[i]);
...
if (!this.doRender) return true;
if (this.haveBsSlabGhost) {
if (!(this.doRender = this.g3d.setColix (this.mesh.slabColix))) return true;
...
if (this.volumeRender && !this.isTranslucent) colix = org.jmol.util.Colix.getColixTranslucent (colix, true, 0.8);
this.colix = colix;
...
if (this.frontOnly && this.transformedVectors[this.normixes[i]].z < 0) continue ;for (var j = p.length - 1; --j >= 0; ) {
var pt = p[j];
if (bsPoints.get (pt)) continue ;bsPoints.set (pt);
this.g3d.fillSphere (4, this.screens[pt]);
}
}
return ;
}for (var i = this.vertexCount; --i >= 0; ) if (!this.frontOnly || this.transformedVectors[this.normixes[i]].z >= 0) this.g3d.fillSphere (4, this.screens[i]);
...
if (this.haveBsDisplay && (!this.mesh.bsDisplay.get (iA) || !this.mesh.bsDisplay.get (iB) || !this.mesh.bsDisplay.get (iC))) continue ;if (iB == iC) {
this.drawLine (iA, iB, fill, this.vertices[iA], this.vertices[iB], this.screens[iA], this.screens[iB]);
continue ;}var check;
if (this.mesh.isTriangleSet) {
var normix = this.normixes[i];
if (!this.g3d.isDirectedTowardsCamera (normix)) continue ;if (fill) {
if (this.isExport) {
this.g3d.fillTriangle (this.screens[iC], this.colix, normix, this.screens[iB], this.colix, normix, this.screens[iA], this.colix, normix);
...
this.g3d.fillTriangle (this.screens[iA], this.colix, normix, this.screens[iB], this.colix, normix, this.screens[iC], this.colix, normix);
}continue ;}check = vertexIndexes[3];
...
continue ;}var nA = this.normixes[iA];
var nB = this.normixes[iB];
var nC = this.normixes[iC];
check = this.checkNormals (nA, nB, nC);
if (fill && check != 7) continue ;switch (vertexIndexes.length) {
case 3:
if (fill) {
if (generateSet) {
this.bsPolygons.set (i);
continue ;}if (iShowTriangles) {
this.g3d.fillTriangle (this.screens[iA], this.colix, nA, this.screens[iB], this.colix, nB, this.screens[iC], this.colix, nC, 0.1);
continue ;}this.g3d.fillTriangle (this.screens[iA], this.colix, nA, this.screens[iB], this.colix, nB, this.screens[iC], this.colix, nC);
continue ;}this.drawTriangle (this.screens[iA], this.colix, this.screens[iB], this.colix, this.screens[iC], this.colix, check, 1);
continue ;case 4:
var iD = vertexIndexes[3];
var nD = this.normixes[iD];
if (this.frontOnly && (check != 7 || this.transformedVectors[nD].z < 0)) continue ;if (fill) {
if (generateSet) {
this.bsPolygons.set (i);
continue ;}this.g3d.fillQuadrilateral (this.screens[iA], this.colix, nA, this.screens[iB], this.colix, nB, this.screens[iC], this.colix, nC, this.screens[iD], this.colix, nD);
continue ;}this.g3d.drawQuadrilateral (this.colix, this.screens[iA], this.screens[iB], this.screens[iC], this.screens[iD]);
}
}
...
if (this.antialias) diam <<= 1;
if ((check & 1) == 1) this.g3d.fillCylinder (colixA, colixB, 1, diam, screenA.x, screenA.y, screenA.z, screenB.x, screenB.y, screenB.z);
if ((check & 2) == 2) this.g3d.fillCylinder (colixB, colixC, 1, diam, screenB.x, screenB.y, screenB.z, screenC.x, screenC.y, screenC.z);
if ((check & 4) == 4) this.g3d.fillCylinder (colixA, colixC, 1, diam, screenA.x, screenA.y, screenA.z, screenC.x, screenC.y, screenC.z);
} else {
this.g3d.drawTriangle (screenA, colixA, screenB, colixB, screenC, colixC, check);
}}, "javax.vecmath.Point3i,~N,javax.vecmath.Point3i,~N,javax.vecmath.Point3i,~N,~N,~N");
...
this.diameter = Math.round ((this.viewer.unscaleToScreen (this.pt1i.z, this.diameter) * 1000));
}if (iA == iB) {
this.g3d.fillSphere (this.diameter, sA);
} else {
...
this.pt1f.scale (0.5);
this.viewer.transformPtScr (this.pt1f, this.pt1i);
var mad = Math.round ((Math.abs (this.width) * 1000));
...
}}, "~N,~N,~B,javax.vecmath.Point3f,javax.vecmath.Point3f,javax.vecmath.Point3i,javax.vecmath.Point3i");
