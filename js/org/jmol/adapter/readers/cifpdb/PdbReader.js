...
var serial = (isAtom ? this.parseInt (this.line, 6, 11) : 0);
...
if (pt < 0 || pt > 0 && (this.line.charAt (pt - 1)).charCodeAt (0) == ('\\').charCodeAt (0)) pt = this.line.length;
...
var nAtoms = 0;
for (var j = chain.length - 1; --j >= 0; ) if ((chain.charAt (j)).charCodeAt (0) == (':').charCodeAt (0)) nAtoms += this.chainAtomCounts[chain.charAt (j + 1).charCodeAt (0)];
...
if (this.checkFilter ("BIOMOLECULE " + iMolecule + ";")) {
...
if ((ch).charCodeAt (0) != (' ').charCodeAt (0)) atom.alternateLocationID = ch;
...
if (!this.filterAtom (atom, this.fileAtomIndex++)) return ;
...
this.setAtomCoord (atom, this.parseFloat (this.line, 30, 40), this.parseFloat (this.line, 40, 50), this.parseFloat (this.line, 50, 60));
} else {
var charge = 0;
if (this.lineLength >= 80) {
var chMagnitude = this.line.charAt (78);
var chSign = this.line.charAt (79);
if ((chSign).charCodeAt (0) >= ('0').charCodeAt (0) && (chSign).charCodeAt (0) <= ('7').charCodeAt (0)) {
var chT = chSign;
chSign = chMagnitude;
chMagnitude = chT;
}if (((chSign).charCodeAt (0) == ('+').charCodeAt (0) || (chSign).charCodeAt (0) == ('-').charCodeAt (0) || (chSign).charCodeAt (0) == (' ').charCodeAt (0)) && (chMagnitude).charCodeAt (0) >= ('0').charCodeAt (0) && (chMagnitude).charCodeAt (0) <= ('7').charCodeAt (0)) {
charge = (chMagnitude).charCodeAt (0) - ('0').charCodeAt (0);
if ((chSign).charCodeAt (0) == ('-').charCodeAt (0)) charge = -charge;
...
function (atom, iAtom) {
if (!Clazz.superCall (this, org.jmol.adapter.readers.cifpdb.PdbReader, "filterAtom", [atom, iAtom])) return false;
if (this.configurationPtr > 0) {
...
var msg = " atom [" + atom.group3 + "]" + atom.sequenceNumber + ((atom.insertionCode).charCodeAt (0) == ('\0').charCodeAt (0) ? "" : "^" + atom.insertionCode) + ((atom.chainID).charCodeAt (0) == ('\0').charCodeAt (0) ? "" : ":" + atom.chainID) + "." + atom.atomName + "%" + atom.alternateLocationID + "\n";
...
if (this.isPQR) {
if (this.gromacsWideFormat) {
atom.partialCharge = this.parseFloat (this.line.substring (60, 68));
atom.radius = org.jmol.adapter.readers.cifpdb.PdbReader.fixRadius (this.parseFloat (this.line.substring (68, 76)));
} else {
var tokens = this.getTokens ();
var pt = tokens.length - 2 - (this.line.length > 75 ? 1 : 0);
atom.partialCharge = this.parseFloat (tokens[pt++]);
atom.radius = org.jmol.adapter.readers.cifpdb.PdbReader.fixRadius (this.parseFloat (tokens[pt]));
}return ;
}var floatOccupancy;
if (this.gromacsWideFormat) {
floatOccupancy = this.parseFloat (this.line.substring (60, 68));
atom.bfactor = org.jmol.adapter.readers.cifpdb.PdbReader.fixRadius (this.parseFloat (this.line.substring (68, 76)));
} else {
floatOccupancy = this.parseFloat (this.line, 54, 60);
...
if ((ch76).charCodeAt (0) == (' ').charCodeAt (0) && org.jmol.adapter.smarter.Atom.isValidElementSymbol (ch77)) return "" + ch77;
...
if ((this.htElementsInCurrentGroup == null || this.htElementsInCurrentGroup.get (this.line.substring (12, 14)) != null) && org.jmol.adapter.smarter.Atom.isValidElementSymbolNoCaseSecondChar (ch12, ch13)) return (isHetero || (ch12).charCodeAt (0) != ('H').charCodeAt (0) ? "" + ch12 + ch13 : "H");
if ((ch12).charCodeAt (0) == ('H').charCodeAt (0)) return "H";
...
if ((ch12).charCodeAt (0) != (' ').charCodeAt (0) && (this.htElementsInCurrentGroup == null || this.htElementsInCurrentGroup.get ("" + ch12) != null) && org.jmol.adapter.smarter.Atom.isValidElementSymbol (ch12)) return "" + ch12;
var ch14 = this.line.charAt (14);
if ((ch12).charCodeAt (0) == (' ').charCodeAt (0) && (ch13).charCodeAt (0) != ('X').charCodeAt (0) && (this.htElementsInCurrentGroup == null || this.htElementsInCurrentGroup.get (this.line.substring (13, 15)) != null) && org.jmol.adapter.smarter.Atom.isValidElementSymbolNoCaseSecondChar (ch13, ch14)) return "" + ch13 + ch14;
return "Xx";
}, "~B");
...
var targetSerial = (offsetEnd <= this.lineLength ? this.parseInt (this.line, offset, offsetEnd) : -1);
...
endChainIDIndex = 31;
endIndex = 33;
if (this.line.length >= 40) substructureType = org.jmol.adapter.smarter.Structure.getHelixType (this.parseInt (this.line.substring (38, 40)));
...
endChainIDIndex = 32;
endIndex = 33;
strandCount = this.parseInt (this.line.substring (14, 16));
...
var serialID = this.parseInt (this.line.substring (7, 10));
var startChainID = this.line.charAt (startChainIDIndex);
var startSequenceNumber = this.parseInt (this.line, startIndex, startIndex + 4);
var startInsertionCode = this.line.charAt (startIndex + 4);
var endChainID = this.line.charAt (endChainIDIndex);
var endSequenceNumber = this.parseInt (this.line, endIndex, endIndex + 4);
...
var iModel = this.parseInt (this.line, startModelColumn, endModelColumn);
...
this.setSpaceGroupName (org.jmol.adapter.smarter.AtomSetCollectionReader.parseTrimmed (this.line, 55, 66));
}, $fz.isPrivate = true, $fz));
...
($fz = function () {
var groupName = this.parseToken (this.line, 12, 15);
var formula = org.jmol.adapter.smarter.AtomSetCollectionReader.parseTrimmed (this.line, 19, 70);
...
formula = org.jmol.adapter.smarter.AtomSetCollectionReader.parseTrimmed (formula, ichLeftParen + 1, ichRightParen);
...
}var groupName = this.parseToken (this.line, 7, 10);
if (this.htHetero.containsKey (groupName)) {
return ;
}var hetName = org.jmol.adapter.smarter.AtomSetCollectionReader.parseTrimmed (this.line, 30, 70);
...
if (this.htHetero == null) {
this.htHetero =  new java.util.Hashtable ();
}var groupName = this.parseToken (this.line, 11, 14);
var hetName = org.jmol.adapter.smarter.AtomSetCollectionReader.parseTrimmed (this.line, 15, 70);
if (groupName == null) {
...
data[6] = 1;
var serial = this.parseInt (this.line, 6, 11);
var index;
...
for (var i = 28, pt = 0; i < 70; i += 7, pt++) data[pt] = this.parseFloat (this.line, i, i + 7);
...
}var nResidues = this.parseInt (this.line, 15, 17);
var siteID = org.jmol.adapter.smarter.AtomSetCollectionReader.parseTrimmed (this.line, 11, 14);
...
for (var i = 0; i < 4; i++) {
var pt = 18 + i * 11;
var resName = org.jmol.adapter.smarter.AtomSetCollectionReader.parseTrimmed (this.line, pt, pt + 3);
if (resName.length == 0) break;
var chainID = org.jmol.adapter.smarter.AtomSetCollectionReader.parseTrimmed (this.line, pt + 4, pt + 5);
var seq = org.jmol.adapter.smarter.AtomSetCollectionReader.parseTrimmed (this.line, pt + 5, pt + 9);
var iCode = org.jmol.adapter.smarter.AtomSetCollectionReader.parseTrimmed (this.line, pt + 9, pt + 10);
...
try {
var tokens = org.jmol.adapter.smarter.AtomSetCollectionReader.getTokens (this.line.substring (10).$replace (':', ' '));
...
this.tlsGroupID = this.parseInt (tokens[tokens.length - 1]);
...
nGroups = this.parseInt (tokens[tokens.length - 1]);
if (nGroups < 1) break;
...
res1 = this.parseInt (tokens[3]);
res2 = this.parseInt (tokens[5]);
} else {
var toC = components.indexOf (" C ");
var fromC = components.indexOf (" C ", toC + 4);
chain1 = this.line.charAt (fromC);
chain2 = this.line.charAt (toC);
res1 = this.parseInt (this.line.substring (fromC + 1, toC));
res2 = this.parseInt (this.line.substring (toC + 1));
...
continue ;}var resno = this.parseInt (tokens[i]);
if (resno == -2147483648) continue ;range =  new java.util.Hashtable ();
range.put ("residues", [resno, this.parseInt (tokens[++i])]);
if ((chain).charCodeAt (0) != ('\0').charCodeAt (0)) range.put ("chains", "" + chain + chain);
ranges.add (range);
}
...
if (tokens.length == 8) {
origin.set (this.parseFloat (tokens[5]), this.parseFloat (tokens[6]), this.parseFloat (tokens[7]));
} else {
var n = this.line.length;
origin.set (this.parseFloat (this.line.substring (n - 27, n - 18)), this.parseFloat (this.line.substring (n - 18, n - 9)), this.parseFloat (this.line.substring (n - 9, n)));
...
var ti = (tokens[i].charAt (0)).charCodeAt (0) - ('1').charCodeAt (0);
var tj = (tokens[i].charAt (1)).charCodeAt (0) - ('1').charCodeAt (0);
...
if ((tensorType).charCodeAt (0) == ('S').charCodeAt (0) && ++iGroup == nGroups) {
...
this.atomSetCollection.setAtomSetAuxiliaryInfo ("TLS", tlsGroupInfo, iModel);
