...
Clazz.load (["org.jmol.adapter.readers.quantum.MopacSlaterReader", "javax.util.BitSet"], "org.jmol.adapter.readers.quantum.MoldenReader", ["java.lang.Double", "$.Exception", "$.Float", "java.util.ArrayList", "$.Hashtable", "org.jmol.api.JmolAdapter", "org.jmol.util.Logger"], function () {
...
this.bsAtomOK =  new javax.util.BitSet ();
this.bsBadIndex =  new javax.util.BitSet ();
});
...
if (this.line.indexOf ("[MO]") == 0) return (!this.readMolecularOrbitals || this.readMolecularOrbitals ());
...
continue ;}for (var j = this.bsAtomOK.nextClearBit (0); j >= 0; j = this.bsAtomOK.nextClearBit (j + 1)) {
...
if (tokens.length < 6) continue ;var atom = this.atomSetCollection.addNewAtom ();
...
var gaussianPtr = 0;
var nCoef = 0;
while (this.readLine () != null && !((this.line = this.line.trim ()).length == 0 || (this.line.charAt (0)).charCodeAt (0) == 91)) {
var tokens = this.getTokens ();
...
var orbData =  Clazz.newArray (nTokens, 0);
...
while (this.readLine () != null && this.line.indexOf ('[') < 0) {
var tokens = this.getTokens ();
if (tokens.length != 4) continue ;var atom = this.atomSetCollection.addNewAtom ();
atom.atomName = tokens[0];
