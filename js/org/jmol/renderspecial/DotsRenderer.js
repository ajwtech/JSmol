...
Clazz.load (["org.jmol.render.ShapeRenderer"], "org.jmol.renderspecial.DotsRenderer", ["javax.vecmath.Vector3f", "org.jmol.shapespecial.Dots", "org.jmol.util.Colix", "$.Geodesic"], function () {
...
for (var i = this.screenDotCount; --i >= 0; ) this.verticesTransformed[i] =  new javax.vecmath.Vector3f ();

this.screenCoordinates =  Clazz.newArray (3 * this.screenDotCount, 0);
});
...
var sppa = Math.round (this.viewer.getScalePixelsPerAngstrom (true));
...
var map = maps[i];
if (map == null || !atom.isVisible (this.myVisibilityFlag) || !this.g3d.isInDisplayRange (atom.screenX, atom.screenY)) continue ;try {
...
while (--iDot >= 0) {
if (!visibilityMap.get (iDot)) continue ;var vertex = this.verticesTransformed[iDot];
if (this.faceMap != null) this.faceMap[iDot] = i;
this.screenCoordinates[i++] = x + Math.round (((scaledRadius * vertex.x) + (vertex.x < 0 ? -0.5 : 0.5)));
this.screenCoordinates[i++] = y + Math.round (((scaledRadius * vertex.y) + (vertex.y < 0 ? -0.5 : 0.5)));
this.screenCoordinates[i++] = z + Math.round (((scaledRadius * vertex.z) + (vertex.z < 0 ? -0.5 : 0.5)));
++nPoints;
}
return nPoints;
