...
Clazz.load (["org.jmol.util.MeshSurface", "javax.vecmath.Point3f", "$.Vector3f"], "org.jmol.shape.Mesh", ["java.lang.Boolean", "$.Float", "$.StringBuffer", "java.util.BitSet", "$.Hashtable", "javax.vecmath.Matrix3f", "$.Matrix4f", "org.jmol.g3d.Graphics3D", "org.jmol.script.Token", "org.jmol.util.ArrayUtil", "$.BitSetUtil", "$.Escape", "$.Measure"], function () {
...
if (this.haveXyPoints) for (var i = this.normixCount; --i >= 0; ) this.normixes[i] = 9999;
 ...
 else for (var i = this.normixCount; --i >= 0; ) this.normixes[i] = org.jmol.g3d.Graphics3D.getNormix (normals[i], bsTemp);

this.lighting = 1073741958;
if (this.insideOut) this.invertNormixes ();
this.setLighting (lighting);
}, "~N,~A,javax.vecmath.Point4f");
...
for (var i = this.normixCount; --i >= 0; ) normals[i] =  new javax.vecmath.Vector3f ();
...
($fz = function () {
for (var i = this.normixCount; --i >= 0; ) this.normixes[i] = org.jmol.g3d.Graphics3D.getInverseNormix (this.normixes[i]);

}, $fz.isPrivate = true, $fz));
Clazz.defineMethod (c$, "setTranslucent", 
function (isTranslucent, iLevel) {
this.colix = org.jmol.g3d.Graphics3D.getColixTranslucent (this.colix, isTranslucent, iLevel);
}, "~B,~N");
...
try {
if (!this.setABC (i)) continue ;var vA = vertices[this.iA];
...
if (this.meshColix != 0) s.append (" color mesh ").append (org.jmol.g3d.Graphics3D.getHexCode (this.meshColix));
...
return isWithinNot;
}, "javax.vecmath.Point3f,java.util.List,~N,~B");
