...
this.linearOffsets =  Clazz.newArray (8, 0);
});
...
this.cubeCountY = (this.nY = volumeData.voxelCounts[1]) - 1;
this.cubeCountZ = (this.nZ = volumeData.voxelCounts[2]) - 1;
this.yzCount = volumeData.getYzCount ();
...
this.yzPlanes = (this.mode == 3 ?  Clazz.newArray (2, this.yzCount, 0) : null);
...
function () {
for (var i = 8; --i >= 0; ) this.volumeData.transform (org.jmol.jvxl.calc.MarchingCubes.cubeVertexVectors[i], this.voxelVertexVectors[i] =  new javax.vecmath.Vector3f ());

for (var i = 12; --i >= 0; ) this.edgeVectors[i].sub (this.voxelVertexVectors[org.jmol.util.TriangleData.edgeVertexes[i + i + 1]], this.voxelVertexVectors[org.jmol.util.TriangleData.edgeVertexes[i + i]]);
...
}if (this.bsExcludedPlanes.get (x) && this.bsExcludedPlanes.get (x + xStep)) continue ;if (this.colorDensity) {
...
continue ;}var indexPlane = this.isoPointIndexPlanes[0];
...
if (insideMask == 0) {
++outsideCount;
continue ;}if (insideMask == 0xFF) {
++insideCount;
continue ;}++surfaceCount;
...
if ((edgeMask & xEdge) == 0) continue ;var iPlane = this.edgeVertexPlanes[iEdge];
...
continue ;}var vertexA = org.jmol.util.TriangleData.edgeVertexes[iEdge << 1];
...
this.volumeData.voxelPtToXYZ (x, y, z, this.pt0);
pt.add (this.pt0, this.voxelVertexVectors[vertex]);
}, "~N,~N,~N,~N,javax.vecmath.Point3f");
...
c$.cubeVertexVectors = c$.prototype.cubeVertexVectors = [ new javax.vecmath.Vector3f (0, 0, 0),  new javax.vecmath.Vector3f (1, 0, 0),  new javax.vecmath.Vector3f (1, 0, 1),  new javax.vecmath.Vector3f (0, 0, 1),  new javax.vecmath.Vector3f (0, 1, 0),  new javax.vecmath.Vector3f (1, 1, 0),  new javax.vecmath.Vector3f (1, 1, 1),  new javax.vecmath.Vector3f (0, 1, 1)];
Clazz.defineStatics (c$,
