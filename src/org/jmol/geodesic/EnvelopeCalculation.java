...
public final class EnvelopeCalculation {
  ...
  private BitSet emptySet = new BitSet();
...
//    geodesicCount = Geodesic.getVertexVectorsCount();
  ...
  public static BitSet allocateBitmap(int bitCount) {
  ...
  public BitSet newBitSet(BitSet bitsetToCopy) {
  	return BitSetUtil.copy(bitsetToCopy);
  }
  ...
  public Point3f[] getPoints() {
    ...
    for (int i = dotsConvexMax; --i >= 0;)
      if (dotsConvexMaps[i] != null)
        nPoints += cardinality(dotsConvexMaps[i],dotCount);
  ...
  private int cardinality(BitSet bitmap, int max) {
      int n = bitmap.cardinality();
      for (int i = bitmap.length(); --i >= max;)
        if (bitmap.get(i))
          n--;
      return n;
	}
