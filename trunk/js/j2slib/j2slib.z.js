...
Clazz.haveInt32 = (typeof Int32Array != "undefined");
/**
 * Make arrays.
 *
 * @return the created Array object
...
Clazz.newInt32Array  = function () {
	...
	if (args.length <= 1) {
		...
		for (var i = 0; i < dim; i++) {
			// Call recursively!
			arr[i] = Clazz.newInt32Array (xargs);
		}
		return arr;
	}
};
...
ClazzLoader.xhrOnload = function (transport, file) {
	if (transport.status >= 400 || transport.responseText == null
		...
		if (fs[file] == null) {
			...
			alert ("[Java2Script] ClazzLoader.xhrOnload Error in loading " + file + "!");
		}
...
ClazzLoader.generatingW3CScriptOnCallback = function (path, forError) {
	return function () {
	if (forError)alert("############ forError=" + forError + " path=" + path + " ####" + (forError ? "NOT" : "") + "LOADED###");
	...
	return function () {
		...
		if ((local || state == "loaded")
			...
			if (!local && (fss[path] == null || fss[path] == 0)
				...
				alert ("[Java2Script] generatingIEScriptOnCallback Error in loading " + path + "!");
			}
		}
		if (ClazzLoader.inLoadingThreads > 0) {
			ClazzLoader.inLoadingThreads--;
...
var isTest=false;// (node.name && (node.name.indexOf("Shader") > 0 || ("" + node).indexOf("Col") > 0));
	if (node.name == null 
