...
this.setColor (value == null ? 0 : org.jmol.util.Colix.getColix (value));
return ;
...
if (len < 2 || len > 4) continue ;var v =  Clazz.newArray (len + 1, 0);
v[0] = len;
...
switch (md.tokAction) {
case 12291:
this.define (-2147483648, pt, true, false, false);
this.setIndices ();
break;
case 1048589:
this.showHide (pt, false);
break;
case 1048588:
this.showHide (pt, true);
break;
case 1060866:
this.$delete (pt);
this.toggle (pt);
...
this.showHide ( new org.jmol.modelset.Measurement (this.modelSet, value, null, null), true);
return ;
}if ("reformatDistances" === propertyName) {
this.reformatDistances ();
return ;
}if ("show" === propertyName) {
this.showHide ( new org.jmol.modelset.Measurement (this.modelSet, value, null, null), false);
return ;
...
if ("pointInfo".equals (property)) return this.measurements.get (Math.floor (index / 10)).getLabel (index % 10, false, false);
...
var i = this.find (m);
var mt;
if (i >= 0 && !(mt = this.measurements.get (i)).isHidden ()) this.define (i, mt, true, false, false);
 else this.define (-1, m, false, true, false);
this.setIndices ();
...
this.radiusData = null;
var i = this.find (m);
if (i >= 0) this.define (i, this.measurements.get (i), true, false, false);
this.setIndices ();
}, $fz.isPrivate = true, $fz), "org.jmol.modelset.Measurement");
Clazz.defineMethod (c$, "$delete", 
($fz = function (value) {
if (Clazz.instanceOf (value, Array)) {
this.define (-2147483648,  new org.jmol.modelset.Measurement (this.modelSet, value, null, null), true, false, false);
return ;
}if ((Clazz.instanceOf (value, Integer))) this.deleteMeasurement ((value).intValue ());
...
if (iPt >= 0) this.deleteMeasurement (iPt);
return ;
}this.defineMeasurement (iPt, m, doSelect);
return ;
}if (isShow) {
this.define (iPt, m, true, false, false);
if (isDelete) return ;
...
Clazz.defineMethod (c$, "deleteMeasurement", 
($fz = function (i) {
...
var m = this.measurements.get (i);
m.setVisible (false);
if (this.mad == 0 || m.isHidden ()) continue ;for (var iAtom = m.getCount (); iAtom > 0; iAtom--) {
var atomIndex = m.getAtomIndex (iAtom);
if (atomIndex >= 0) {
if (!this.modelSet.atoms[atomIndex].isClickable ()) continue out;} else {
...
var s = org.jmol.shape.Shape.getShapeCommands (temp, null, "select measures");
