...
if (!Clazz.isClassDefined ("org.jmol.modelsetbio.AlphaPolymer")) {
...
proteinstructure =  new org.jmol.modelsetbio.Helix (this, indexStart, structureCount, 0, type);
break;
case org.jmol.constant.EnumStructure.SHEET:
proteinstructure =  new org.jmol.modelsetbio.Sheet (this, indexStart, structureCount, 0, type);
break;
case org.jmol.constant.EnumStructure.TURN:
proteinstructure =  new org.jmol.modelsetbio.Turn (this, indexStart, structureCount, 0);
break;
default:
...
var biopolymerStartsEnds =  Clazz.newArray (nBiopolymers, nEndMin * 2, 0);
for (var i = 0; i < n; i++) {
a1 = vCA.get (i);
...
var d2 =  Clazz.newArray (Math.floor (n * (n - 1) / 2), 0);
for (var i = 0; i < n; i++) {
a1 = vCA.get (i);
for (var j = i + 1; j < n; j++) {
...
var pt = (Math.floor (k / nEndMin)) * nEndMin + j;
if ((i = biopolymerStartsEnds[b][pt] - 1) >= 0) bsStruts.set (i);
biopolymerStartsEnds[b][pt] = -1;
}
}}
if (biopolymerStartsEnds[b][0] == -1 && biopolymerStartsEnds[b][nEndMin] == -1) continue ;var okN = false;
var okC = false;
var iN = 0;
var jN = 0;
var iC = 0;
...
var i = biopolymerStartsEnds[b][k] - 1;
if (i == -2) {
k = (Math.floor (k / nEndMin) + 1) * nEndMin - 1;
continue ;}if (j == i || i == -1) continue ;var ipt = org.jmol.modelsetbio.AlphaPolymer.strutPoint (i, j, n);
if (bsNearbyResidues.get (ipt) || d2[ipt] > (k < nEndMin ? minN : minC)) continue ;if (k < nEndMin) {
...
if (k1 == k2) {
continue ;}var ipt = org.jmol.modelsetbio.AlphaPolymer.strutPoint (k1, k2, n);
...
($fz = function () {
var angles =  Clazz.newArray (this.monomerCount, 0);
...
this.addSecondaryStructure (tag, null, 0, 0, i, iMax - 1);
i = iMax;
}
}, $fz.isPrivate = true, $fz), "~A");
c$.$AlphaPolymer$ = function () {
Clazz.pu$h ();
c$ = Clazz.declareType (org.jmol.modelsetbio, "AlphaPolymer", Enum);
