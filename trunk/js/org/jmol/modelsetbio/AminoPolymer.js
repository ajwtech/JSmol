...
function () {
for (var i = 0; i < this.monomerCount - 1; ++i) this.calcPhiPsiAngles (this.monomers[i], this.monomers[i + 1]);
...
var vNH =  new javax.vecmath.Vector3f ();
var source;
var min1 = (min == null ?  Clazz.newArray (2, 3, 0) : null);
...
if (polymer === this && (i == indexDonor || i + 1 == indexDonor)) continue ;var target = polymer.monomers[i];
var oxygen = target.getCarbonylOxygenAtom ();
if (oxygen == null || bsB != null && !bsB.get (oxygen.index)) continue ;var targetAlphaPoint = target.getLeadAtom ();
var dist2 = sourceAlphaPoint.distanceSquared (targetAlphaPoint);
if (dist2 >= 81.0) continue ;var energy = this.calcHbondEnergy (sourceNitrogenPoint, hydrogenPoint, target, checkDistances);
if (energy < min[0][2]) {
m = min[1];
min[1] = min[0];
min[0] = m;
...
continue ;}m[0] = polymer.bioPolymerIndexInModel;
...
}, $fz.isPrivate = true, $fz), "javax.vecmath.Point3f,javax.vecmath.Point3f,org.jmol.modelsetbio.AminoMonomer,~B");
...
var min =  Clazz.newArray (bioPolymerCount, 0);
for (var i = 0; i < bioPolymerCount; i++) {
if (!(Clazz.instanceOf (bioPolymers[i], org.jmol.modelsetbio.AminoPolymer))) continue ;var n = bioPolymers[i].monomerCount;
min[i] =  Clazz.newArray (n, 2, 3, 0);
for (var j = 0; j < n; ++j) {
...
if (bsBad.get (ia)) continue ;for (var p2 = p1; p2 < min.length; p2++) if (Clazz.instanceOf (bioPolymers[p2], org.jmol.modelsetbio.AminoPolymer)) for (var b = (p1 == p2 ? a + 3 : 1); b < min[p2].length - 1; b++) {
var ap2 = bioPolymers[p2];
var ib = ap2.monomers[b].leadAtomIndex;
if (bsBad.get (ib)) continue ;if ((bridge = org.jmol.modelsetbio.AminoPolymer.getBridge (min, p1, a, p2, b, bridgesP, atoms[ia], atoms[ib], ap1, ap2, vHBonds, htTemp, false, htLadders)) != null) {
...
continue ;}if (org.jmol.util.Logger.debugging) org.jmol.util.Logger.debug ("Bridge found " + bridge);
bsDone[p1].set (a);
bsDone[p2].set (b);
...
var bridge = ap1Clazz.innerTypeInstance (org.jmol.modelsetbio.AminoPolymer.Bridge, this, null, atom1, atom2, htLadders);
bridges.add (bridge);
if (vHBonds != null) {
...
}, $fz.isPrivate = true, $fz), "org.jmol.modelsetbio.AminoPolymer.Bridge,java.util.Map,java.util.Map,~B,~N,~N");
...
}, $fz.isPrivate = true, $fz), "org.jmol.modelsetbio.AminoPolymer.Bridge,java.util.Map,java.util.Map,~B,~N");
...
var structureTags =  Clazz.newArray (this.monomerCount, '\0');
...
if (end >= start + 3) {
this.addSecondaryStructure (org.jmol.constant.EnumStructure.HELIX, null, 0, 0, start, end);
}start = end;
}}
for (var start = 0; start < this.monomerCount; ++start) {
...
if (end >= start + 2) {
this.addSecondaryStructure (org.jmol.constant.EnumStructure.SHEET, null, 0, 0, start, end);
}start = end;
}}
for (var start = 0; start < this.monomerCount; ++start) {
...
if (end >= start + 2) {
this.addSecondaryStructure (org.jmol.constant.EnumStructure.TURN, null, 0, 0, start, end);
}start = end;
}}
}, "~B");
...
c$.$AminoPolymer$Bridge$ = function () {
Clazz.pu$h ();
c$ = Clazz.decorateAsClass (function () {
...
Clazz.instantialize (this, arguments);
}, org.jmol.modelsetbio.AminoPolymer, "Bridge");
Clazz.makeConstructor (c$, 
function (a, b, c) {
this.a = a;
this.b = b;
this.ladder =  Clazz.newArray (2, 2, 0);
...
function (a, b) {
if (a == null || a.isAntiparallel != this.isAntiparallel || !this.canAdd (a) || !a.canAdd (this)) return false;
this.extendLadder (a.ladder[0][0], a.ladder[1][0]);
this.extendLadder (a.ladder[0][1], a.ladder[1][1]);
a.ladder = this.ladder;
if (a.ladder !== this.ladder) {
b.remove (a.ladder);
this.addLadder (b);
}return true;
}, "org.jmol.modelsetbio.AminoPolymer.Bridge,java.util.Map");
Clazz.defineMethod (c$, "canAdd", 
($fz = function (a) {
var b = a.a.index;
var c = a.b.index;
return (this.isAntiparallel ? (b >= this.ladder[0][1] && c <= this.ladder[1][0] || b <= this.ladder[0][0] && c >= this.ladder[1][1]) : (b <= this.ladder[0][0] && c <= this.ladder[1][0] || b >= this.ladder[0][1] && c >= this.ladder[1][1]));
}, $fz.isPrivate = true, $fz), "org.jmol.modelsetbio.AminoPolymer.Bridge");
Clazz.defineMethod (c$, "extendLadder", 
($fz = function (a, b) {
if (this.ladder[0][0] > a) this.ladder[0][0] = a;
if (this.ladder[0][1] < a) this.ladder[0][1] = a;
if (this.ladder[1][0] > b) this.ladder[1][0] = b;
if (this.ladder[1][1] < b) this.ladder[1][1] = b;
}, $fz.isPrivate = true, $fz), "~N,~N");
c$ = Clazz.p0p ();
};
