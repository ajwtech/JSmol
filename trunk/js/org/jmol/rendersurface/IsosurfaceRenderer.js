...
Clazz.load (["org.jmol.render.MeshRenderer", "javax.vecmath.Point3f", "$.Point3i"], "org.jmol.rendersurface.IsosurfaceRenderer", ["java.lang.Boolean", "$.Float", "javax.vecmath.Vector3f", "org.jmol.util.Colix", "$.Normix"], function () {
...
this.ptTempi =  new javax.vecmath.Point3i ();
});
...
if (this.isNavigationMode) mySlabValue = Math.round (this.viewer.getNavigationOffset ().z);
this.isosurface.keyXy = null;
for (var i = this.isosurface.meshCount; --i >= 0; ) {
this.imesh = this.isosurface.meshes[i];
if (this.imesh.connections != null && !this.viewer.getModelSet ().atoms[this.imesh.connections[0]].isVisible (0)) continue ;this.hasColorRange = false;
...
var dy = Math.floor (Math.floor (height / 2) / (n - 1));
var y = Math.floor (height / 4) * 3 - dy;
var x = 10 * factor;
var dx = 20 * factor;
this.isosurface.keyXy = [Math.floor (x / factor), 0, Math.floor ((x + dx) / factor), Math.floor ((y + dy) / factor), Math.floor (dy / factor)];
...
this.isosurface.keyXy[1] = Math.floor ((y + dy) / factor);
...
var r = this.viewer.scaleToScreen (Math.round (this.pt2f.z), Math.round (points[0].distance (points[1])) * 500);
...
var v = vContours[i];
if (v.size () < 6) continue ;this.colix = (this.imesh.meshColix == 0 ? (v.get (3))[0] : this.imesh.meshColix);
...
diam = Math.floor (this.viewer.getScreenDim () / (this.volumeRender ? 50 : 100));
...
this.g3d.volumeRender (diam, this.screens[i].x, this.screens[i].y, this.screens[i].z);
} else {
this.g3d.fillSphere (diam, this.screens[i]);
}}
if (incr == 3) {
this.g3d.setColix (this.isTranslucent ? org.jmol.util.Colix.getColixTranslucent (12, true, 0.5) : 12);
for (var i = 1; i < this.vertexCount; i += 3) this.g3d.fillCylinder (3, Math.floor (diam / 4), this.screens[i], this.screens[i + 1]);

this.g3d.setColix (this.isTranslucent ? org.jmol.util.Colix.getColixTranslucent (21, true, 0.5) : 21);
for (var i = 1; i < this.vertexCount; i += 3) this.g3d.fillSphere (diam, this.screens[i]);

this.g3d.setColix (this.isTranslucent ? org.jmol.util.Colix.getColixTranslucent (7, true, 0.5) : 7);
...
if (polygon == null || this.haveBsSlabDisplay && !this.bsSlab.get (i)) continue ;var iA = polygon[0];
var iB = polygon[1];
var iC = polygon[2];
if (this.imesh.jvxlData.thisSet >= 0 && this.imesh.vertexSets != null && this.imesh.vertexSets[iA] != this.imesh.jvxlData.thisSet) continue ;if (this.haveBsDisplay && (!this.imesh.bsDisplay.get (iA) || !this.imesh.bsDisplay.get (iB) || !this.imesh.bsDisplay.get (iC))) continue ;var nA = this.normixes[iA];
var nB = this.normixes[iB];
var nC = this.normixes[iC];
var check = this.checkNormals (nA, nB, nC);
if (fill && check == 0) continue ;var colixA;
var colixB;
var colixC;
if (colorSolid) {
if (colorArrayed && i < this.imesh.polygonColixes.length) {
var c = this.imesh.polygonColixes[i];
if (c == 0) continue ;colix = c;
...
if (colixA != colixB || colixB != colixC) continue ;if (this.haveBsSlabGhost) colixA = colixB = colixC = org.jmol.util.Colix.copyColixTranslucency (this.imesh.slabColix, colixA);
}}if (diam == -2147483648) {
if (this.imesh.diameter <= 0) {
diam = this.viewer.getMeshScale ();
} else {
diam = Math.floor (this.viewer.getScreenDim () / 100);
}if (diam < 1) diam = 1;
}if (fill) {
if (generateSet) {
...
if (iA == iB) this.g3d.fillSphere (diam, this.screens[iA]);
...
continue ;}this.drawTriangle (this.pt1i, colix, this.pt2i, colix, this.pt3i, colix, check, diam);
}}
